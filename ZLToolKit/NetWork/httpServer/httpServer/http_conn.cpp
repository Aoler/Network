...
void addfd( int epollfd, int fd, bool one_shot )
{
    ...
    EV_SET(&ev,fd,EVFILT_READ,flags, 0, 0, (void*)(intptr_t)fd);
    int r = kevent(epollfd, &ev, 1, NULL, 0, NULL);
    exit_if(r, "kevent failed ");
    setNonBlock( fd );
}
...
{
    struct kevent ev;
    EV_SET(&ev, fd, 0, EV_DELETE, 0, 0, (void*)(intptr_t)fd);
    int r = kevent(epollfd, &ev, 1, NULL, 0, NULL);
    exit_if(r, "kevent failed ");
    close( fd );
}
...
void modfd( int epollfd, int fd, int filter )
{
    struct kevent ev;
    EV_SET(&ev, fd,filter,EV_ADD|EV_ENABLE|EV_CLEAR|EV_ONESHOT, 0, 0, (void*)(intptr_t)fd);
    int r = kevent(epollfd, &ev, 1, NULL, 0, NULL);
    exit_if(r, "kevent failed ");
}
...
void http_conn::init( int sockfd, const sockaddr_in& addr )
{
    ...
    //updateEvents(m_epollfd,sockfd,kReadEvent|kWriteEvent,false);
    m_user_count++;
...
bool http_conn::write()
{
    ...
    {
        //modfd( m_epollfd, m_sockfd, EPOLLIN );
        modfd( m_epollfd, m_sockfd, EVFILT_READ );
        init();
        return true;
    }
    ...
    while( 1 )
    {
        ...
        if ( temp <= -1 )
        {
            ...
            if( errno == EAGAIN )
            {
                ...
                modfd( m_epollfd, m_sockfd, EVFILT_WRITE );
                return true;
            }
